  ...
  up: async (queryInterface, Sequelize) => {
    ...
    */
    ...
    filemaps.map((el) => {
      ...
      let init_type = (_.isNaN(index_element) || _.max(init_max) === 0 ? 0 : index_element > 6 ? "DRIVEN" : "MOTOR")
    ...
    let numberOfweek = moment(el.initial_date,"MM-DD-YYYY").week();
    let year = moment(el.initial_date,"MM-DD-YYYY").year()
    let month = moment(el.initial_date,"MM-DD-YYYY").month() + 1
    let week_code = month.toString()+""+year.toString()+numberOfweek.toString();
    
    let two_date = moment(el.initial_date).subtract(7,'d').format('YYYY-MM-DD');
    ...
    let three_date = moment(el.initial_date).subtract(14,'d').format('YYYY-MM-DD');
    ...
    let four_date = moment(el.initial_date).subtract(21,'d').format('YYYY-MM-DD');
    ...
    let week_one = (_.isEmpty(el.initial_date) ? '-' : `${el.EquipmentID.toString()}${month.toString()}${year.toString()}${numberOfweek.toString()}` )
    let week_two = (_.isEmpty(el.initial_date) ? '-' : `${el.EquipmentID.toString()}${two_myears.toString()}${two_number.toString()}`)
    let week_three = (_.isEmpty(el.initial_date) ? '-' : `${el.EquipmentID.toString()}${three_myears.toString()}${three_number.toString()}`)
    let week_four = (_.isEmpty(el.initial_date) ? '-' : `${el.EquipmentID.toString()}${four_myears.toString()}${four_number.toString()}`)
